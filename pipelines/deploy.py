...
def main():
    ...
    # Give the underlying SageMaker Model object a versioned name so each
    # deployment registers a distinct model in the console.
    ...
    if endpoint_exists(sm, args.endpoint_name):
        ...
        # Register the model first (deploy() would do this internally on create,
        # but for an update we need the model name & config name explicitly).
